...
export function removeNodes<T>(customs: Normalized<T>[]): void {
  customs.forEach((curr) => removeNode(curr.element));
}

export function insertNodes<T>(customs: Normalized<T>[]): void {
...
export function handleDOMChanges<T>(customs: Normalized<T>[]): void {
  removeNodes(customs);
  insertNodes(customs);
}

export function createCustoms<T>(
  evt: SortableEvent,
  list: T[],
): Normalized<T>[] {
  const custom = [
...
export function handleStateChanges<T>(
...
export function handleStateRemove<T>(
  normalized: Normalized<T>[],
  list: T[],
): T[] {
  const newList = [...list];
...
export function handleStateAdd<T>(
  normalized: Normalized<T>[],
  list: T[],
...
export function createNormalized<T>(
  inputs: Input[],
  list: T[],
): Normalized<T>[] {
